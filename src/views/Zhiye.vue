<template>
  <div class="page-content" v-loading="loading">
    <router-link :to="{ name: 'ZhiyeCreate' }">
      <el-button size="medium" type="primary" plain>创建职业</el-button>
    </router-link>
    <el-table class="mt-20" :data="zhiyes" size="medium">
  ...
  methods: {
    ...
    handleCommand(command) {
      ...
      if (type === "edit") {
        ...
        this.delZhiyes(index);
      }
    },
    delZhiyes(index) {
      const id = this.zhiyes[index].id;
      this.$confirm("确认删除吗？")
        .then(() => {
          return serviceZhiye.destory(id);
        })
        .then(() => {
          this.$message.success("删除课程成功");
          this.zhiyes.splice(index, 1);
        })
        .catch(() => {});
    }
  }
